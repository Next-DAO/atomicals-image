...
export class ResolveCommand implements CommandInterface {
  ...
  async run(): Promise<any> {
    ...
    if (atomicalType.type === AtomicalIdentifierType.ATOMICAL_ID || atomicalType.type === AtomicalIdentifierType.ATOMICAL_NUMBER) {
      cmd = new GetCommand(this.electrumApi, atomicalType.providedIdentifier || '', this.fetchType, this.path, this.verbose);
    } else if (atomicalType.type === AtomicalIdentifierType.REALM_NAME) {
      cmd = new GetByRealmCommand(this.electrumApi, atomicalType.realmName || '', this.fetchType, this.path, this.verbose);
    } else if (atomicalType.type === AtomicalIdentifierType.CONTAINER_NAME) {
      cmd = new GetByContainerCommand(this.electrumApi, atomicalType.containerName || '', this.fetchType, this.path, this.verbose);
    } else if (atomicalType.type === AtomicalIdentifierType.TICKER_NAME) {
      cmd = new GetByTickerCommand(this.electrumApi, atomicalType.tickerName || '', this.fetchType, this.path, this.verbose);
    }
